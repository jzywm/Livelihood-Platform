...
- **设计基线已定档**：PDD v1.19（`docs/design/产品设计文档.md`；v1.0 于 2026-09-11 定档，v1.19 为 2026-09-16 习惯归属裁决后的最新版）、《微服务边界与职责基准》v2.0（`docs/design/微服务边界与职责基准.md`）、《高并发架构演进设计》v1.0（`docs/design/高并发架构演进设计.md`，同批定档）、15 个服务的 README + 14 份 openapi.yaml + 14 份 er.md（`services/*/docs/`）均已评审定档——本设计**不产生新架构决策**（唯一例外见 D10），全部技术选择引用这些定档口径。
...
### D10 用户习惯与购买影响因素（长期记忆）归属（引用边界基准 §4.16-C16、PDD §4.4/§5.11.1.4、需求 28 §12）

- **决策**：**PROFILE 为长期记忆类数据的唯一权威存储**（`profile_habit` / `profile_habit_factor` / `behavior_daily_agg`，落 ⑥ 画像域共享主库、`profile_` 前缀隔离）；**AICORE 只算不存**（`POST /aicore/habit/summarize` 无状态计算，不建记忆表），算法迭代不触发数据迁移。
- **为什么不是 AICORE**：习惯语义上是画像延伸（需求 28 §3 已含品类偏好/价格敏感度），四权、个保法、PIA 义务都在 PROFILE；放 AICORE 会产生**两份真相**，用户点「删除画像」时残留即四权破功。
- **为什么加日聚合层**：若无 `behavior_daily_agg`，每次习惯总结需搬运原始行为事件约 **4000 万条/天**（200 万 MAU × 90 天窗口口径）；按 账户×日×类型×品类 预聚合后降至约 **200 万条/天（约 1/20）**，是「无状态计算」方案可行的前提。
- **为什么不引入 PostgreSQL**（用户裁决 D1=否）：画像类数据量级为「600 万用户 × 每用户数十行、点查为主」，MySQL 充分胜任；引入 PG 的唯一技术理由（按天分区 + `DROP PARTITION`）可被「Redis 承接短期会话（30 分钟 TTL）+ MySQL 承接长期画像」组合替代，而代价（多一套备份/容灾/信创/运维口径）真实存在。**短期记忆归 Redis、长期记忆归 MySQL，二者不同生命周期**。
- **表达与红线**：只存「权重 + 样本量 + 可解释证据」，**不存人格推断标签**；`weight` 保持明文（密文无法参与 SQL 排序），若 PIA 更严仅单独加密 `evidence`；权重**只进推荐排序、不进定价链路**（R-07/C7 不杀熟）；样本来源 `sampleSource` 如实标注（M1=`BEHAVIOR` 行为近似，M2 交易接入后升 `PURCHASE`/`MIXED`）。
- **合规管辖澄清**：R-06「不留跨会话记忆」针对**对话原文**；习惯属 J-07 画像类数据，走最小化 + 授权可撤回 + 四权 + 删除后 30 天清理——**不在 R-06 管辖范围内，故 PRD R-06 与 J4-2 隐私承诺不变**。
- **备选（未选）**：① 全部落在 AICORE 的 PG 库（改动最小，但产生第二份画像真相、四权需两处实现）；② 仅 PROFILE 自算、AICORE 完全不参与（最简，但失去「算法迭代不动数据」的解耦收益）。
