...
| 配置 | `gateway.*`(白名单/内部路径/限流初值/运维网段/**令牌策略** `auth.access-token-max-ttl` + `auth.clock-skew`)、`spring.cloud.gateway.server.webflux.routes`(路由;**acc 路由带 `PreserveHostHeader` 过滤器 = 保留原始 Host**,R-A16)、`resilience4j.circuitbreaker.configs.default.*`(熔断) |
...
| `acc.session.allowed-origins` | 默认留空即可（**同源放行**，裁定 R-A15；默认链路已由 **R-A16** 闭合）；**跨源前端**（前端 origin ≠ 入口 origin）时才填前端入口 origin（逗号分隔） | 漏配的后果只发生在「来源 ≠ ACC 看到的请求自身来源」时:浏览器发起的换发/仅凭 Cookie 登出被来源校验拒为 **401 + 2001**（登录可用、换发不可用）。**网关已保留原始 Host（R-A16）**：acc 路由启用 SCG 的 `PreserveHostHeader` 过滤器,入口按常规 `proxy_set_header Host $host` 透传即可,ACC 由 `Host` 推断出浏览器看到的 host ⇒ 同源默认放行在默认拓扑下**自动生效**,本项无需为本拓扑维护。**若链路中另有入口代理,建议同时 `proxy_set_header X-Forwarded-Host $host;`**（`nginx-gateway.conf.example` 已给出该行,作双保险）；**HTTPS 入口（TLS 在入口终结）仍需透传 `X-Forwarded-Proto $scheme`**——scheme 不能由 `Host` 推断（样例已含该行）。历史发现（R-A16 之前）:SCG 默认把 Host 改写成 ACC 内网地址且不转发原始 Host,该拓扑下必须二选一——① 列出前端入口 origin,② 让入口设置 `X-Forwarded-Host $host`；该约束**已由网关保留 Host 解决** |
...
| 15.7 | **R-A16 默认链路来源校验**:入口只透传 `Host`(如 `Host: api.example.com`)、浏览器同源 `Origin`(**不带任何** `X-Forwarded-*`)经网关换发 | 200 + 新 Cookie(ACC 由保留的 `Host` 推断出同源);同一形态 + `Origin: https://evil.example` → 401 + 2001 且**不轮换**(真机取证见 `services/acc/deploy/drill/README.md` §3.1 的 3.10/3.11/3.12) |
