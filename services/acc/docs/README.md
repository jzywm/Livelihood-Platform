...
用户通过微信 / 支付宝实名回传完成注册，获得平台账户与六方角色。流程：一键实名 → 授权协议 → 回传实名信息（姓名 / 身份证号）→ 返回 accountId + realNameStatus。异常处理：第三方不可用即暂停；信息不匹配报 3002；重复 open_id 幂等返回原账户。
