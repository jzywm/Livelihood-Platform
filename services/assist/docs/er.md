...
erDiagram
    ...
    %% RECOMMEND_CANDIDATE/GUIDE_COMPARISON 取数：CRED(信用分/放心供应商标识) + TRACE(溯源完整度) + PROFILE(授权画像权重 + 习惯/购买影响因素权重,J-09 叠加排序)
    %%   习惯权重权威存储与四权均在本服务之外（PROFILE `profile_habit` / `profile_habit_factor`,2026-09-16 增）；本服务只读用于排序,价格与画像无关（不杀熟）
    %% 会话上下文经 ACC(实名/账户) 鉴权取数;人工入口走 TICKET(G-01 热线/工单)
    %% LLM(DeepSeek 开放平台)仅作对话/意图/流程编排,平台硬护栏在编排之外强制执行,不开放自由工具/函数调用
    %% 会话口径不变:30 分钟 TTL、不留跨会话记忆（R-06）；**本服务不掌握长期记忆**——习惯为 J-07 画像类数据,归 PROFILE
```
...
- **会话语义**：`session_id` 缺省新建；30 分钟无操作自动清空（TTL）；**不留跨会话记忆**；`/assist/session/clear` 手动清空。**会话口径 2026-09-16 复核后保持不变**——「不留跨会话记忆」（R-06）指对话原文；用户习惯与购买影响因素属 J-07 画像类数据、由 PROFILE 唯一权威存储（边界基准 v2.0 §4.16-C16），**本服务只读其排序权重、不持有该数据**。
- **推荐取数（2026-09-16 增）**：推荐/导购的画像依据除职业/价格敏感度外，还包含 PROFILE 的习惯权重与购买影响因素权重（`/profile/recommend` 一并返回）；理由模板须标注画像依据（含习惯依据，可解释）；**权重只影响排序、不影响价格**（R-07/C7 不杀熟）。
