...
> 依据文档：`docs/design/产品设计文档.md` v1.0（基线）（§5.9 / §6.4.8 / §8.3.1）、`docs/design/微服务边界与职责基准.md` v1.6（§2.9 / §3 / §4.5）、
> `docs/design/高并发架构演进设计.md` v1.0（§2.1~§2.6）、`services/trade/docs/openapi.yaml` v1.0.0（**唯一可手改源**）。
> 数据域归属：⑧ 交易域（`order`、`escrow`，M2）。分账记录 `split_record` 已按 §4-C07 决策（2026-09-08）迁入 ⑨ 结算域 SETTLE——本服务**只触发分账、不落表**（2026-09-11 终审回改）。
> 口径：与 openapi.yaml 冲突时以 openapi.yaml 为准。
> 版本：v1.1 · 2026-09-11（v1.0 首版 2026-09-10：七章齐全；**终审回改**——`split_record` 按 §4-C07 迁出本域（权威在 SETTLE ⑨ 结算域，本服务只触发不落表），交易域 6 表 + Redis 辅助结构；D-04 退货状态机按基线补齐 6 态）。

## 1. ER 图（Mermaid）

```mermaid
erDiagram
    %% TRADE 交易域 ER 图 · 担保交易 + 货款托管 + 分账触发 + 争议仲裁 + 退货退款 + 交易互评 + 上门服务(T-11~T-15/D-04/T-23/A-09)
    %% 分表：order/escrow 按月分表 + 按买家 buyer_id hash 二级(buyer_id + created_at,M2,高并发§2.2)
    %% 权威数据(实名/商品/信用分)经内部接口只读引用；分账记录 split_record 权威在 SETTLE(⑨ 结算域,C07 已决策),本服务经 POST /settle/split/execute 触发；证据附件仅存 OSS 对象键——逻辑关联、非外键
    ...
    ORDER ||--o| SPLIT_RECORD : "验收通过触发分账,1:1(权威在SETTLE⑨结算域,§4-C07)"
    ...
    %% SPLIT_RECORD 为逻辑标注(非本域表):权威在 SETTLE(⑨ 结算域,§4-C07 已决策迁入),本服务经 POST /settle/split/execute 触发、不落表
...
| `split_record` | **SETTLE（⑨ 结算域，本域不落表）** | 分账记录（I-04）：卖方货款 + 平台服务费；权威在 SETTLE（§4-C07 已决策迁入），本服务经 `POST /settle/split/execute` 触发 |
...
- **R-01 不碰货款 = 不二清**：货款托管于持牌机构（`escrow`，非平台账户），平台仅收服务费（`fee_rate` 0.5%~1% 分品类公开，P-01）；`order.total_amount = escrow.amount`（全额托管 → 验收后由 SETTLE 拆分 `goods_amount + fee_amount` 入 `split_record`）；不设平台资金余额字段。
...
- **分表定位**：`order`/`escrow` 按月分表 + 按买家 `buyer_id` hash 二级（分片键 `buyer_id + created_at`，M2，高并发 §2.2 已定）；分账记录 `split_record` 落 SETTLE 结算域（同按月口径，权威在 SETTLE）；其余工作单/主数据不分片（详见 §5）。
...
- **状态机**：订单 `PENDING_PAYMENT → PAID(托管) → SHIPPED(在途) → PENDING_ACCEPTANCE(48h) → COMPLETED(放款+互评)`；分支 `REFUNDING → REFUNDED`、`DISPUTING → CLOSED`；托管 `FROZEN → RELEASED/REFUNDED`；分账状态机（`PENDING → SUCCESS/FAILED`）在 SETTLE；争议 `DISPUTING → ARBITRATED`；退货 `PENDING_REVIEW → APPROVED → RETURNED → REFUNDING → REFUNDED / REJECTED`（D-04 基线 6 态，2026-09-11 补齐）；上门服务 `PENDING_ACCEPT → ACCEPTED → SERVING → COMPLETED`。超时未发货自动催发/可取消、验收超期自动确认（系统动作 JOB/MQ）。
...
| ORDER — SPLIT_RECORD | 1 : 1 | 验收通过触发分账（PENDING→SUCCESS/FAILED）；**split_record 权威在 SETTLE（⑨ 结算域，§4-C07 已决策），本服务只触发不落表** |
...
> 平台级策略以《高并发架构演进设计》v1.0 §2.1~§2.6 为准，本节只做「平台策略 → TRADE 交易域」的落地映射。§2.2 已给本服务 `order`/`escrow` 分表方案，**原样采用、不改口径**（`split_record` 分表方案随 SETTLE 结算域）。
...
| `split_record` | **SETTLE（⑨ 结算域）** | `order_id` + `created_at` | `split_record_YYYYMM` | 同钱包流水 | M2 |
...
> **分片阈值**（平台级建议初值，压测/数据增长标定）：单表 >2000 万行 或 >20GB 触发再分；`order`/`escrow` 按月分表天然可控，冷数据到点即归档，避免单表膨胀到亿级。
> **二级 hash 分片说明**：`order`/`escrow` 在「按月分表（时间一级）」之上，M2 分库时再按买家 `buyer_id` hash 二级分库；`escrow` 冗余 `buyer_id`，随订单落同分片，保证「订单 + 托管」详情聚合不跨分片 JOIN（分账记录 `split_record` 在 SETTLE 同月分表，金额拆分经 `POST /settle/split/execute` 回执留痕，不参与本域聚合）。

### 5.3 分表路由规则

- **路由层**：M1 用 MyBatis-Plus 自定义分表插件（拦截按月路由）+ 统一 `ShardingKey` 注解；M2 分库后评估 ShardingSphere-Proxy（业务零改造），或先自研轻量路由。
- **写入**：按 `created_at` 月份路由到 `{表}_YYYYMM`，`ShardingKey(buyer_id, created_at)` 显式声明；`escrow` 以订单 `buyer_id` + 自身 `created_at` 落同分片。
- **查询**：订单列表/详情（GET /trade/orders、GET /trade/orders/{orderId}）**必须携带 `buyer_id` 下推**（分片键剪枝 + 水平越权校验）；订单详情聚合 {order, escrow} 按 `order_id` + `buyer_id` 逐表下推、同月直查；分账状态经 SETTLE 接口查询/回执（`split_record` 可能跨月），聚合走异步/从库。
...
| `split_record.split_id` | bigint 雪花 ID | `infra-idgen` | API 输出 `spl_` 前缀（权威在 SETTLE） |
...
- **待评审（§5.7）**：`O-`/`D-`/`R-` 前缀在 openapi 示例为可读业务号（`O-YYYYMMDD-SEQ`），是否改用号段（`id_allocator`）生成连续可读单号（对齐工单编号 `TKT-YYYYMMDD-XXXX` 场景），或以雪花 + 前缀直出，待评审；`esc_`/`trev_` 为雪花 + 前缀（无争议；`spl_` 前缀随分账记录归 SETTLE）。

### 5.5 读写分离与冷热归档

- **读写分离**：主库写、从库读（读是写 3 倍以上）；读请求经 `@ReadOnly` 注解路由到从库；关键资金「写后立即读」（下单后立即查托管状态、验收/退款后立即查状态）**强制走主库**，避免主从延迟读到旧状态。
- **冷热归档**：`order`/`escrow` >12 月热转冷 OSS（Parquet/压缩），查询走归档快照；对账/审计按需回捞。
- **存证**：托管只增不改 + `escrow.hash` 哈希链（P-02），归档前后哈希链不断（分账流水存证在 SETTLE，同 P-02 口径）。
...
| `split_record` 归属 | ✅ 已定档：已按 §4-C07 迁 ⑨ 结算域 SETTLE（本服务不落表，只触发分账；2026-09-11 终审回改） | 已定档 |
| 退货状态机完整枚举 | ✅ 已定档：D-04 基线 6 态 `PENDING_REVIEW → APPROVED → RETURNED → REFUNDING → REFUNDED / REJECTED`（2026-09-11 补齐 openapi/数据字典） | 已定档 |
...
> 验收结果（AcceptType：ACCEPTED/PARTIAL_REJECT/FULL_REJECT）与后续动作（action：RELEASE/REFUND）为验收接口瞬态值，不单独落列，其结果反映在 `status` 与 `escrow`/退款记录中（分账记录在 SETTLE；如需审计留痕加列，§5.7）。
...
### 6.3 split_record（分账记录，权威在 SETTLE ⑨ 结算域，本域不落表）

> **本服务不设计该表**（§4-C07 已决策迁 ⑨ 结算域 SETTLE）。分账记录结构见 `services/settle/docs/er.md` §6.5（`split_id` 雪花 ID `spl_` 前缀、`order_id` 1:1、`goods_amount`/`fee_amount` 金额 string 输出、状态机 PENDING→SUCCESS/FAILED、按月分表 `split_record_YYYYMM`）；本服务验收通过后经 `POST /settle/split/execute` 触发，拒收/退货经分账反向退款，查询经 SETTLE 接口回执。
...
| status | enum('PENDING_REVIEW','APPROVED','RETURNED','REFUNDING','REFUNDED','REJECTED') | NO | — | PENDING_REVIEW | 退货状态机（D-04 基线 6 态，2026-09-11 定档）：申请待审核 → 审核通过 → 已寄回/已上门取 → 退款中 → 退款完成；驳回（REJECTED，自 PENDING_REVIEW） |
...
- **用途**：担保交易订单主数据（T-11/T-12/T-14）——下单托管、发货、验收、退款、争议的状态机载体；订单详情聚合 {order, escrow} + SETTLE 分账回执（PDD §6.4.8）。
...
### 7.3 split_record（分账记录，权威在 SETTLE，本域不落表）

- **用途**：分账记录（I-04）——卖方货款 + 平台服务费拆分留痕；**权威在 SETTLE（⑨ 结算域，§4-C07 已决策迁入），本服务只触发、不落表**（结构/索引/约束见 `services/settle/docs/er.md` §6.5/§7.5）。
- **接口映射**：GET /trade/orders/{orderId}（详情内嵌 SplitInfo，数据经 SETTLE 查询）；出方向 POST /settle/split/execute（触发分账，x-external-interfaces）。
...
- **约束**：退货状态机（D-04 基线 6 态：PENDING_REVIEW→APPROVED→RETURNED→REFUNDING→REFUNDED / REJECTED，2026-09-11 定档）；驳回必填 `review_reason`；退款经 SETTLE 分账反向；敏感操作（退款）强制 MFA（2003）；非已完成状态不可退货（3007）。
...
- **分账执行**：SETTLE（`POST /settle/split/execute`）——验收通过触发、拒收/退货反向退款；**分账记录 `split_record` 权威在 SETTLE（⑨ 结算域，§4-C07 已决策），本服务只触发不落表**。
...
*文档结束 · 与 `services/trade/docs/openapi.yaml`（唯一可手改源）、《高并发架构演进设计》v1.0 §2、《产品设计文档》v1.0（基线） §5.x/§6.4.x、《微服务边界与职责基准》v1.6 §2.x 同步维护。*
