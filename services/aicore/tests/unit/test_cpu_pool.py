...
1. 工单 §2.5b 逐字：「sleep 只允许出现在**专门验证线程边界**的那条用例，且时长 ≤ 0.15s」；
   本文件的值都在这条线内：CPU 函数 `time.sleep(0.1)`、定时协程 `asyncio.sleep(0.01)`。
   每一处都带 `# ai-allow-sleep: <理由>`（验收脚本第 4 项的判据支持行级豁免，理由必填）。
2. **等待是判据本身的一部分**：「事件循环不被阻塞」只能靠"同时发生的两件事谁先完成"
   来判定，这一点无法用注入时钟替代——被注入的时钟不会真的把 `time.sleep` 放进线程里。
   故这些等待不是"测试凑合等一会儿"，而是被测行为的载体。
3. **"本文件是全套测试里唯一有真等待的地方"这句话是错的**（本轮更正）：
   `tests/unit/test_task_runner.py`（观察心跳是否仍在续期）与
   `tests/integration/test_lease_redis.py`（真实 `PX` 到期）各自都有，
   理由分别写在那两个文件的 docstring 里。此处 MUST NOT 再写成"唯一"。

睡眠豁免：3 处（机械判据逐文件比对，见 `tests/structural/test_sleep_exemption_counts.py`）。
"""
...
async def test_event_loop_keeps_running_while_cpu_work_is_in_flight() -> None:
    ...
    判据取**完成顺序**（同一个 `order` 列表里的先后）而不是耗时数字：耗时断言会在慢机器上假红。

    ## 它靠的是**时间余量**（N3 更正：此前这里写"顺序不受机器快慢影响"）

    顺序之所以能判，是因为两个时长差了 **10 倍**（CPU 100ms vs 定时 10ms）：
    只要 100ms 还没跑完而 10ms 已经到点，顺序就是 `["timer", "cpu"]`。
    这**不是**"不受机器快慢影响"，而是"余量足够大"——在极端抢占（CI 上被挂起 100ms 以上）
    时它仍可能翻。之所以保留而不是改成条件等待：本用例要证的正是
    「**同时发生的两件事谁先完成**」，而这个"同时"必须由两段真实的时长造出来
    （被注入的时钟不会真的把 `time.sleep` 放进线程里），改成条件等待会把被测行为本身改掉。
    故这里**如实写成"靠 10× 余量"**，并在极端环境下接受它可能假红。
    """
    order: list[str] = []
