...
# 连接池两项（Task 3.4）在列：容量是**部署决策**，给默认值等于替运维做决定，
# 且「以为配了其实没配」会表现为连接耗尽——故与其它必填项同一档。
# 只读地址两项**刻意不在列**：它们是可选能力（None = 没有独立从库），
# 若定为必填，任何单实例环境都得把主库地址再写一遍——那是形式主义而非安全。
REQUIRED_FIELDS = (
    "mysql_host",
    "mysql_user",
    "mysql_password",
...
BASE_ENV: dict[str, str] = {
    ...
    # 连接池两项必填，故必须在基线里；只读地址两项**刻意不铺**：
    # 「没配从库」（None → 回落主库）是默认分支，由默认值用例与只读用例各自显式断言。
...
def test_env_example_is_a_usable_template(monkeypatch: pytest.MonkeyPatch) -> None:
    """模板必须**能直接用**：清掉进程环境后按 `.env.example` 构造 `Settings` 必须成功。

    这条比「名字一一对应」更强：它连带校验模板里的**取值**都合法——等价于运维
    `cp .env.example .env` 之后（除真实凭据外不填任何东西）服务能起来。
    Task 3.4 的连接池两项必填、只读两项可空，四种字段类型因此在模板里各有一行可校验。
    """
    _clear_aicore_env(monkeypatch)
    ...
    assert settings.mysql_readonly_host == "your_readonly_host"
    assert settings.read_target_is_primary is False
...
def test_defaults_are_applied_when_only_required_values_given(
    ...
    assert settings.mysql_pool_size == 5  # 来自环境变量（必填项：没有默认值可断言）
    assert settings.mysql_max_overflow == 10
    # 只读地址两项有默认值：None = 没有独立从库（可空字段的默认值正是「回落」本身）。
    assert settings.mysql_readonly_host is None
    assert settings.mysql_readonly_port is None
    assert settings.read_target_is_primary is True
...
INVALID_BOUNDS: tuple[tuple[str, object], ...] = (
    ...
    ("mysql_readonly_port", 0),
    ("mysql_readonly_port", 70000),
    ("redis_port", 0),
    ("redis_port", 70000),
    ("redis_db", -1),
...
VALID_BOUNDS: tuple[tuple[str, object], ...] = (
    ...
    # 只读端口必须**与只读主机成对**：单配端口会被 `readonly-pair` 跨字段规则拒绝
    # （控制者裁定，理由见 test_readonly_port_without_host_is_rejected），
    # 故边界值用例同时在 `env_extra` 里补上主机，验的仍是"端口边界被接受"这件事。
    ("mysql_readonly_port", 1),
    ("mysql_readonly_port", 65535),
    ("redis_port", 1),
    ("redis_port", 65535),
    ("redis_db", 0),
...
def test_boundary_values_are_accepted(
    ...
    `mysql_readonly_port` 的两条会额外带上只读主机：该端口不能单独配置
    （`readonly-pair` 跨字段规则，见 `test_readonly_port_without_host_is_rejected`），
    但"端口边界是否被接受"这件事仍要被验到，故把配对项补齐而不是删掉这两条。
    """
    extra = {"mysql_readonly_host": "10.20.30.40"} if field == "mysql_readonly_port" else {}
    settings = build_settings(monkeypatch, **{field: value}, **extra)
...
# ---------------------------------------------------------------------------
# 只读（从库）地址（Task 3.4）：可空、回落只认 None、回落必须「看得见」
# ---------------------------------------------------------------------------


def test_readonly_address_is_optional_and_falls_back_to_primary(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """未配只读地址时只读目标就是主库，且**回落事实可断言**（`read_target_is_primary`）。

    这条属性是给日志与自检用的：本机只有一个实例，回落是常态，
    MUST NOT 把「回落主库」说成（或让调用方误以为）「已分离」。
    """
    settings = build_settings(monkeypatch)

    assert settings.read_target_is_primary is True
    assert settings.mysql_read_host == settings.mysql_host
    assert settings.mysql_read_port == settings.mysql_port
    # 只读 DSN 与主库 DSN 在「没配从库」时逐字相同——这正是回落的形式化表达。
    assert settings.mysql_read_dsn == settings.mysql_dsn


def test_readonly_address_overrides_the_read_target(monkeypatch: pytest.MonkeyPatch) -> None:
    """配了只读地址就按其取值：host / port 与主库不同，且回落标志为 False。"""
    settings = build_settings(
        monkeypatch, mysql_readonly_host="10.20.30.40", mysql_readonly_port=3307
    )

    assert settings.read_target_is_primary is False
    assert settings.mysql_read_host == "10.20.30.40"
    assert settings.mysql_read_port == 3307
    assert settings.mysql_read_dsn != settings.mysql_dsn
    assert "10.20.30.40" in settings.mysql_read_dsn
    assert "3307" in settings.mysql_read_dsn


def test_readonly_port_without_host_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    """只配只读端口、不配只读主机 → **拒绝启动**（控制者裁定，取代"端口生效"）。

    这里原本断言的是「端口与地址各自独立回落，单独配端口 MUST NOT 被静默丢弃」，
    其论证（`[跨字段：readonly-pair]` 落地时重写过这条用例）是：
    「落到『主库 host + 指定端口』是个错配置，但静默忽略端口更糟」——**该论证成立**，
    但它只看了两个选项。还有第三个：**既不静默忽略、也不去连错地址，而是明确拒绝**。

    裁定理由（三条，逐条可核）：
    1. 「主库地址 + 非默认端口」**没有任何部署会要这个连接**；
       它最可能的原因是"想配从库却漏了 host"。
    2. 静默接受的实际后果是**运行期连不上**，而根因（漏配 host）在日志里看不出来；
       拒绝则把"漏配"变成启动期一句话，运维一轮就能定位。
    3. 代价极小：多写一行配置。

    **仍然保持"不静默忽略"的原则**——拒绝本身就是最强的"不忽略"。
    注意与「只配 host」的区别：那个组合**合法**（同端口不同主机的从库是常见拓扑），
    由 `test_readonly_address_overrides_the_read_target` 覆盖（它配了 host 与 port，
    断言只读端口取到配置值）；「只配 host、端口回落主库」则由
    `test_readonly_address_is_optional_and_falls_back_to_primary` 的端口口径间接守住。
    """
    with pytest.raises(ValidationError) as excinfo:
        build_settings(monkeypatch, mysql_readonly_port=3307)

    errors = excinfo.value.errors()
    assert errors[0]["loc"] == (), "配对约束是跨字段规则，loc 应为空元组"
    assert "[跨字段：readonly-pair]" in errors[0]["msg"]
    # 消息必须点出"要怎么改"，而不只是"不行"
    assert "AICORE_MYSQL_READONLY_HOST" in errors[0]["msg"]


@pytest.mark.parametrize("blank", ["", "   "])
def test_blank_readonly_host_is_rejected_at_construction(
    monkeypatch: pytest.MonkeyPatch, blank: str
) -> None:
    """空串 / 纯空白 MUST NOT 被当成「没配从库」：构造期即判为字段级错误。

    与 `None` 的区别是刻意的：`None` 是「没有独立从库」，空串是「配了但配错了」。
    静默回落会让配错重新表现为「看起来正常」。
    """
    with pytest.raises(ValidationError) as excinfo:
        build_settings(monkeypatch, mysql_readonly_host=blank)

    errors = excinfo.value.errors()
    assert ("mysql_readonly_host",) in [err["loc"] for err in errors], (
        f"空串必须被判为 mysql_readonly_host 的字段级错误，实际：{errors}"
    )
    assert "不是写一个空值" in str(excinfo.value) or "是两回事" in str(excinfo.value), (
        f"错误信息必须说清「空串 ≠ 没配」，实际：{excinfo.value}"
    )


def test_blank_readonly_host_is_rejected_even_when_validation_is_bypassed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """绕过构造校验（`model_copy` 不校验）后，派生属性仍 MUST NOT 静默回落。

    字段校验器只管「构造 Settings」这条路径；`model_copy(update=...)` / 直接改属性
    都不经过它。若属性侧只按 `is None` 判断，空串就会悄悄变成「回落主库」——
    配错重新表现为正常。故两条路径各判一次。
    """
    clean = build_settings(monkeypatch)
    bypassed = clean.model_copy(update={"mysql_readonly_host": "  "})

    with pytest.raises(ValueError, match="回落只认 None"):
        _ = bypassed.mysql_read_host


def test_mysql_read_dsn_names_the_read_target_without_password(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """只读 DSN 与主库 DSN 同口径：含 host / user / database，MUST NOT 含口令。"""
    dsn = build_settings(monkeypatch, mysql_readonly_host="10.20.30.40").mysql_read_dsn

    assert "10.20.30.40" in dsn
    assert "test_user" in dsn
    assert "aicore_test" in dsn
    assert "test_password" not in dsn, f"只读 DSN 泄漏了口令：{dsn}"


def test_mysql_read_dsn_does_not_change_with_password(monkeypatch: pytest.MonkeyPatch) -> None:
    """阴性对照：只改口令，只读 DSN 同样逐字不变。"""
    first = build_settings(monkeypatch, mysql_readonly_host="10.20.30.40")
    monkeypatch.setenv("AICORE_MYSQL_PASSWORD", "test_password_rotated")
    second = Settings(_env_file=None, mysql_readonly_host="10.20.30.40")

    assert second.mysql_password == "test_password_rotated"
    assert second.mysql_read_dsn == first.mysql_read_dsn
