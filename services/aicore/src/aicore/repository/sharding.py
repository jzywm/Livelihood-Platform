...
class CrossShardOperationError(ParamError):
    ...
    **也不取业务规则段的码**：跨分片是**参数组合**问题（`1xxx` 段），不是业务规则失败，
    张冠李戴会让前端按错的语义处置。

    > 勘误（第 3 组独立评审指出）：本段原有一句「何况 `AICORE_ERROR_CODES` 里根本没有 `3007`」，
    > **已过期** —— Task 3.7 接入 `DuplicateVerdictError` 时已按四处清单把 3007 收进该集合
    > （见 `core/errors.py` 的 `CONFLICT_CODE` / `ConflictError`）。
    > 但**本类的码值不变**：那句话原本只是旁证，定档理由是上面那两条语义论证。
