...
> 依据文档：`docs/design/产品设计文档.md` v1.12（§5.12 / §6.4.13 / §8.3.1）、`docs/design/微服务边界与职责基准.md` v1.2（§2.12 / §4.5）、
> `docs/design/高并发架构演进设计.md` v0.3（§2.1~§2.6、§3 异步削峰、ADR-7）、`services/aicore/docs/openapi.yaml` v1.0.0（**唯一可手改源**）。
> 数据域归属：**独立库 `aicore`**（自建，与主站库隔离，M1 起；服务自有库原则，边界基准 v1.2 §2.12）；权威数据（A-02 档案/信用分、A-08 预警）仍在 CRED/DASH，经内部接口事件回写（不复制权威数据）。
...
- **R-05 / C8 AI 只标记不决策**：任何执法/处罚/公示动作必须人工确认（`review_verdict.action`）；置信度分级 HIGH≥0.9 / MEDIUM 0.7~0.9 / LOW<0.7（建议初值，上线前评审、阈值可调）；AI 输出风险建议、人工决策执行；不自动处罚。
...
> 平台级策略以《高并发架构演进设计》v0.3 §2.1~§2.6 / §3 / ADR-7 为准，本节做「平台策略 → AICORE 独立库」的落地映射。
...
| 项 | 建议初值 | 裁决方式 |
|---|---|---|
| 置信度分级阈值 | HIGH ≥0.9 / MEDIUM 0.7~0.9 / LOW <0.7 | 上线前评审，可调 |
| 任务热态镜像 TTL | 7 天 | 运行标定 |
...
*文档结束 · 与 `services/aicore/docs/openapi.yaml`（唯一可手改源）、《高并发架构演进设计》v0.3 §2/§3、《产品设计文档》v1.9 §5.12/§6.4.13、《微服务边界与职责基准》v1.2 §2.12 同步维护。*
